...
class HighMagnitudeQuakeCell: UITableViewCell {
    
    // MARK: - UI Elements(Programatically)
    
    var magnitude : UILabel = {
        let label = UILabel()
        CustomUIElement.forLabel(sender: label, name: Constants.leagueGothicRegularFont, size: 40.0,textColor: .white, textAlignment: .center)
        return label
    }()
    
    var date : UILabel = {
        let label = UILabel()
        CustomUIElement.forLabel(sender: label, name: Constants.leagueGothicRegularFont, size: 20.0,textColor: .white, textAlignment: .left)
        return label
    }()
    ...
    var HighImportanceImage : UIImageView = UIImageView()
    ...
    var item: Earthquakes? {
        didSet {
            ...
            self.HighImportanceImage.image = UIImage(named: Constants.HighImportanceImage)
            self.HighImportanceImage.accessibilityLabel = Constants.HighImportanceImageAccessibility
        }
    }
    ...
    func setupView() {
        ...
        addSubview(time)
        addSubview(depth)
        addSubview(HighImportanceImage)
        setConstraints()
    }
    ...
    func setConstraints() {
        ...
        HighImportanceImage.translatesAutoresizingMaskIntoConstraints = false
        ...
        NSLayoutConstraint.activate([
            HighImportanceImage.widthAnchor.constraint(equalToConstant: 50),
            HighImportanceImage.heightAnchor.constraint(equalToConstant: 50),
            HighImportanceImage.centerYAnchor.constraint(equalTo: centerYAnchor),
            HighImportanceImage.trailingAnchor.constraint(equalTo: trailingAnchor, constant: -15),
...
*/
extension HighMagnitudeQuakeCell {
